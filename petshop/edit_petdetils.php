...
<head>
<style>
#mainact{width:1000px;height:auto;background:#ffffff;margin-left:450px;border:2px solid #1E90FF;}

#mainact h1{text-align:center;color:#fff;background:#6495ED;border-radius:4px;border:2px solid #fff;}
...
<?php
		...
		if($_FILES['petimg1']['tmp_name']=="")
			...
			$query=$con->prepare("update petdetails set pet_name='$petname',pet_type='$pettype',pet_Rate='$petrate',pet_keywords='$petkeyword',pet_features1='$petfeature1',pet_features2='$petfeature2',pet_foods='$petfoods' where pet_id='$petid'");
			
			if($query->execute())
			{
				echo"<script>alert('update details')</script>";
				 echo"<script>window.open('viewallpetdetails.php','_self')</script>";
			
			}
			else
			{
				echo"<script>alert('not update details')</script>";
